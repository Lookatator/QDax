...
from qdax.core.containers.mapelites_repertoire import MapElitesRepertoire, GridRepertoire
...
class MAPElitesGrid(MapElitesRepertoire):
    """Core elements of the MAP-Elites algorithm.
    ...
    @partial(jax.jit, static_argnames=("self", "resolution_desc_tuple"))
    def init(
        self,
        init_genotypes: Genotype,
        ...
        random_key: RNGKey,
    ) -> Tuple[GridRepertoire, Optional[EmitterState], RNGKey]:
        """
        Initialize a Map-Elites repertoire with an initial population of genotypes.
        Requires the definition of centroids that can be computed with any method
        such as CVT or Euclidean mapping.
        ...
        # init the repertoire
        repertoire = GridRepertoire.init(
            genotypes=init_genotypes,
            fitnesses=fitnesses,
            descriptors=descriptors,
    ...
    def update(
        ...
        emitter_state: Optional[EmitterState],
        random_key: RNGKey,
    ) -> Tuple[GridRepertoire, Optional[EmitterState], Metrics, RNGKey]:
        """
        Performs one iteration of the MAP-Elites algorithm.
        1. A batch of genotypes is sampled in the repertoire and the genotypes
            are copied.
